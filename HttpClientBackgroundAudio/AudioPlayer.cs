...
namespace HttpClientBackgroundAudio
{
    public class AudioPlayer : AudioPlayerAgent
    {
        static readonly AudioTrack[] AudioTracks =
        {
            new AudioTrack(new Uri("http://s3.amazonaws.com/hanselminutes/hanselminutes_0414.mp3"), null, null, null, null)
        };
        ...
        static async Task RunDownload()
        {
            try
            {
                Debug.WriteLine("Starting download at " + DateTimeOffset.Now);

                await DownloadAsync().ConfigureAwait(false);

                Debug.WriteLine("Completed download at " + DateTimeOffset.Now);
            }
            catch (Exception ex)
            {
                Debug.WriteLine("Download failed at {0} with {1}", DateTimeOffset.Now, ex.Message);
            }
        }

        static async Task<long> DownloadAsync()
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var stream = await httpClient.GetStreamAsync("http://dds.cr.usgs.gov/pub/data/nationalatlas/elev48i0100a.tif_nt00828.tar.gz").ConfigureAwait(false))
                {
                    for (; ; )
                    {
                        ...
                        using (longTimeout.Token.Register(() =>
                                                                         {
                                                                             Debug.WriteLine("Closing stream at " + DateTimeOffset.Now);

                                                                             // "shortTimeout" did not stop the read.
                                                                             stream.Close();
                                                                         }))
                        using (var shortTimeout = new CancellationTokenSource(TimeSpan.FromSeconds(30)))
                        using (shortTimeout.Token.Register(() => Debug.WriteLine("Cancel at " + DateTimeOffset.Now)))
                        using (var cts = CancellationTokenSource.CreateLinkedTokenSource(longTimeout.Token, shortTimeout.Token))
                        {
                            length = await stream.ReadAsync(buffer, 0, buffer.Length, cts.Token).ConfigureAwait(false);
                        }

                        Debug.WriteLine("Read {0} + {1}", length, totalLength);
